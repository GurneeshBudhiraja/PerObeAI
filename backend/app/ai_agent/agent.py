...
from .agent_tools import retrieve_upperwear, retrieve_lowerwear,invalid_request, format_return_data, get_temperature_by_city
from constants import FLASH_MODEL_001

def agent(user_id:str, user_prompt:str, city:str, preferred_fashion_style:str|None=None)->list:
  try:
    ...
    prompt = ChatPromptTemplate.from_messages([
      ...
      ("human", "{user_prompt} ,user_id is {user_id} ,the city is {city}, user's fashion style is   {preferred_fashion_style}"),
        
      ("placeholder","{agent_scratchpad}")
    ])

    tools = [get_temperature_by_city ,retrieve_upperwear, retrieve_lowerwear, invalid_request, format_return_data]
