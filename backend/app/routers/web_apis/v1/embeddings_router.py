...
from fastapi.responses import JSONResponse # TODO: will remove this later on after centralizing the response and error handling


router = APIRouter(
  prefix = "/api/web/v1", 
  tags = ["embeddings_router"]
  )


@router.post("/generate-embeddings-and-store")
async def generate_and_store_embeddings(
  images: list[dict],
  user_id:str # TODO: will add the depends after debug
  ...
  try:
    ...
    embeddings_count = insert_vectors(images_vector_data=processed_images, user_id=user_id)
    
    # TODO: will change the response later on when the response handling
    return JSONResponse(status_code=status.HTTP_200_OK, content={"message": f"Embeddings count is {embeddings_count}"})
